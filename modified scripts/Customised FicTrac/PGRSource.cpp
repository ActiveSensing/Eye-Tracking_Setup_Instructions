...
bool PGRSource::grab(cv::Mat& frame)
{
    ...
    try {
        ...
        //FENK LAB MODIF (By Yoni)
        // modification to convert the camera timestamp into the system time reference (using 01/01/1970 epoch)
        double ts = ts_ms();
        double cameraTime = pgr_image->GetTimeStamp();
        if (mLastSystemTime == 0)
        {
            mLastSystemTime = ts;
            mLastCameraTime = cameraTime;
            _timestamp = ts;
        }
        else
        {
            double camDuration = cameraTime - mLastCameraTime;
            double sysDuration = ts - mLastSystemTime;
            double mCam2sys = camDuration / sysDuration;
            _timestamp = static_cast<double>(camDuration / mCam2sys) + mLastSystemTime;
            mLastCameraTime = cameraTime;
            mLastSystemTime = ts;
        }
