...
class Projector():
    ...
    def initialize_projection_matrix(self,stim_size, fov_azi, fov_ele, is_fullbowl=False):
        ...
        #up down angle rho
        rhosycenter = 0 if not is_fullbowl else ycenter
        rhos = (np.around((np.sqrt((xmat - xcenter)**2 + (ymat - rhosycenter)**2)) / xcenter * self.stim_y)).astype(int)

        #left right phi
        phis = 0
        if not is_fullbowl:
            phis = (np.around((np.arctan2((ymat - 0), (xmat - xcenter)))/np.pi*self.stim_x)).astype(int)
        else:
            angles = (np.arctan2(ymat - ycenter, xmat - xcenter) + np.pi / 2) % (2 * np.pi)
            phis = (np.around(angles / (2 * np.pi) * self.stim_x)).astype(int)
